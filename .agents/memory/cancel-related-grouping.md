...
**취소 후보 포함 규칙(2026-06-16)**: 접수취소 팝업 연관 후보 필터는 이미 최종 취소된 `접수취소`만
제외하고 `취소대기`는 **포함**(기본 전체선택)해야 함. `취소대기`까지 제외하면 연관건이 모두
취소대기일 때 클릭한 1건만 떠서 한 번에 취소 불가.
**Why**: 취소대기는 아직 미취소 상태라 함께 일괄 취소 대상이어야 함.
