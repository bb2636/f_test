...
export interface MaterialRow {
  ...
  isOverridden?: boolean; // 사용자 수정 여부 (true: 자동 재계산 시 수량/단가/합계 모두 보존)
  ...
  const updateRow = (rowId: string, field: keyof MaterialRow, value: any) => {
    ...
    onRowsChange(rows.map(row => {
      if (row.id === rowId) {
        ...
        // [정책 2026-05-13] 사용자 수정 플래그 분리.
        //   - 수량/단가/합계 수정 → isOverridden=true (전체 보존, 면적 변경에도 갱신 X)
        //   - 자재항목/자재 선택 → isItemOverridden=true (자재명만 보존, 수량은 면적 기반 갱신 유지)
        //   목적: "이후 면적만 변경 시 연동" 정책 만족 + 합지/실크 선택값 회귀 방지.
        const quantityPriceFields: (keyof MaterialRow)[] = ['수량m2', '수량EA', '수량', '단가', '기준단가', '합계', '금액'];
        const itemFields: (keyof MaterialRow)[] = ['자재항목', '자재'];
        if (row.isAutoGenerated && quantityPriceFields.includes(field)) {
          updated.isOverridden = true;
          console.log('[자재비] 자동 생성 행 수량/단가 수정 → isOverridden=true:', row.autoKey, 'field:', field);
        }
        if (row.isAutoGenerated && itemFields.includes(field)) {
          updated.isItemOverridden = true;
          console.log('[자재비] 자동 생성 행 자재항목 선택 → isItemOverridden=true:', row.autoKey, 'value:', value);
        }
      ...
      >
                      ...
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          onChange={(e) => {
                            onRowsChange(rows.map(r => 
                              r.id === row.id 
                                ? { ...r, 자재항목: e.target.value, 자재: e.target.value, 단위: r.단위 || 'm²', ...(r.isAutoGenerated ? { isItemOverridden: true } : {}) }
                                : r
                            ));
                          }}
                          className="h-9 border-0 flex-1"
                                ...
                                onRowsChange(rows.map(r => {
                                  if (r.id === row.id) {
                                    ...
                                    if (isPaint) {
                                      ...
                                      // [중복합산 fix 2026-05-06] 페인트는 수량m2만 사용. 수량EA에도 값을 넣으면
                                      // 단가 입력 핸들러(L844)의 qty=수량m2+수량EA에서 두 배로 합산되어 합계가 2배가 됨.
        ...
        <tfoot>
          <tr style={{ 
            ...
            {/* [정책 2026-05-12] 합계 우측 잔여 셀 — 경비여부 1칸 + 비고(관리자 자재비 구성비) 1칸 (모바일 앱은 비고 셀 숨김) */}
            <td></td>
            {!isMobileApp && (isAdmin ? (
              <td style={{
                padding: "0 12px",
