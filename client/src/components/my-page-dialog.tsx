...
export function MyPageDialog({ open, onOpenChange, user }: MyPageDialogProps) {
  ...
  const menuItems = [
    ...
    ...(user.role !== "협력사" && !(isMobileApp && user.role === "심사사") ? [{ id: "favorites" as TabType, label: "즐겨찾기 목록" }] : []),
  ];
  ...
  const renderInquiryDetail = (inquiry: Inquiry) => (
            ...
            <nav className={isMobileApp ? "flex gap-2 overflow-x-auto floxn-hscroll" : "space-y-1"}>
              {menuItems.map((item) => (
                <button
                  ...
                  style={activeTab === item.id ? { background: '#253396' } : undefined}
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <div className="flex-1 min-w-0">
                                <h4 className="text-sm font-semibold text-gray-900 truncate">{inquiry.title}</h4>
                                <p className="text-xs text-gray-400">{formatDateShort(inquiry.createdAt)}</p>
                              </div>
                              {inquiry.status === "완료" ? (
