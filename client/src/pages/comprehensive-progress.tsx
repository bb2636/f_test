...
const getDisplayApprovedAmount = (c: any): string => {
  ...
  // [정책] 오직 '승인금액'만 표시한다. 견적금액/초기견적으로 대체하지 않는다.
  //   승인금액이 0이면 0 그대로, 값이 없으면 "-"로 표시.
  return formatAmount(c?.approvedAmount);
};
...
export default function ComprehensiveProgress() {
  ...
  const filteredData = useMemo(() => {
    ...
    const filteredDataUnsorted = filteredByStatus.filter((caseItem) => {
      ...
      const assessorId = (caseItem.assessorId || "").toLowerCase();
      const assessorTeam = (caseItem.assessorTeam || "").toLowerCase();
      ...
      return (
        ...
        assessorId.includes(normalizedQuery) ||
        assessorTeam.includes(normalizedQuery)
      );
    });
