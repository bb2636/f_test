    ...
    const rows: GroupedRow[] = entries.map(([accNo, groupCases]) => {
      ...
      return {
        ...
        totalClaim: (() => {
          ...
          // [2026-05-29] 청구금액 중복합산 방지.
          // 손방/대물 청구금액(invoiceDamagePrevention + invoicePropertyRepair)은
          // 케이스 그룹(prefix) 전체가 공유하는 값으로 각 케이스 행에 동일하게 저장됨.
          // 케이스별로 합산하면 같은 청구서가 두 번 더해져 "견적+승인"처럼 두 배로 부풀려짐.
