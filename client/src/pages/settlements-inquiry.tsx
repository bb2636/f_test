  ...
  const tableRows = useMemo(() => {
    ...
    groupedByPrefix.forEach((casesInGroup, prefix) => {
      ...
      combinedRows.push({
        ...
        claimElapsed: (() => {
          const claimDateStr = primaryCase.claimDate;
          if (!claimDateStr || claimDateStr === "-") return "-";
          const hasPaymentCompleted = casesInGroup.some(c => c.paymentCompletedDate);
          if (hasPaymentCompleted) return "-";
          const claimDt = new Date(claimDateStr);
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((row) => {
        ...
        if (numericQuery && !isNaN(Number(numericQuery))) {
          if (
            String(row.claimAmount).includes(numericQuery) ||
            row.claimAmount.toLocaleString().includes(numericQuery) ||
            String(row.settlementDeposit).includes(numericQuery) ||
            row.settlementDeposit.toLocaleString().includes(numericQuery)
          ) {
            return true;
          }
        }
      ...
      >
        ...
        <div className="flex items-end gap-3 flex-wrap">
          ...
          <div className="relative" style={{ flex: "1 1 auto", minWidth: "180px" }}>
            ...
            <Input
              type="text"
              placeholder="검색어를 직접 검색 (청구액, 입금액 포함)"
              value={searchQuery}
                          ...
                          >
                            ...
                            >
                              ...
                              {row.caseIds.map((caseId) => {
                                ...
                                const label = `${baseLabel}${suffix !== "0" && suffix !== "1" ? ` (${suffix})` : ""}`;
                                return (
                                  <button
                                    key={caseId}
                                    onClick={() => {
