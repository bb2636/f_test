  ...
  const renderGroupedRow = (g: GroupedRow) => {
    ...
    return (
      <tr key={g.accidentNo} data-testid={`row-closed-group-${g.accidentNo}`}>
        ...
        <td style={{ ...cellStyle, textAlign: "right" }}>{(() => { const claimDate = getGroupDate(g.cases, "claimDate") || rep.claimDate; if (g.totalClaim && g.totalClaim > 0) return formatAmount(g.totalClaim); if (claimDate) { if (isOnlyPreEstimate(g.cases)) return formatAmount(100000); if (g.totalApproved && g.totalApproved > 0) return formatAmount(g.totalApproved); if (g.totalEstimate && g.totalEstimate > 0) return formatAmount(g.totalEstimate); } return "-"; })()}</td></invoke>
