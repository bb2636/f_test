...
export default function FieldEstimate() {
  ...
  // 허용: 합판, 석고, 도배, 마루, 장판, 상부장, 상부장&하부장, 상부장&키큰장, 상부장&하부장&키큰장, 붙박이장, SMC, 리빙보드
  // 제외: 석고보드(→석고로 교체), 키큰장 단독, 하부장 단독, 도기류, 반자틀, 몰딩, 걸레받이 등
  const DEMOLITION_WORK_NAMES = ['합판', '석고', '도배', '마루', '장판', '상부장', '상부장&하부장', '상부장&키큰장', '상부장&하부장&키큰장', '붙박이장', 'SMC', '리빙보드'];
      ...
      sortedWorkNames.forEach(workName => {
        ...
        if (matchingCatalogItems.length > 0) {
          matchingCatalogItems.forEach((catalogItem, idx) => {
            ...
            if (existingRow) {
              // 기존 행 보존: ID 유지, 면적/수량/금액만 업데이트, 사용자 수정 필드 보존
              newLaborRows.push({
                ...existingRow,
  ...
  const DEMOLITION_REQUIRED_WORK_NAMES = ['합판', '석고', '도배', '마루', '장판'];
