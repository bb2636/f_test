...
export default function FieldEstimate() {
  ...
  const validateAutoSyncGuards = (): { ok: boolean; violations: string[]; warnings: string[] } => {
    ...
    // ⑧ 삭제 키 부활 검증
    //   - 노무비 연동 행은 autoKey가 아니라 makeLinkedLaborDeletionKey
    //     (`category|normalizedWorkName|detailItem`)로 추적된다 (L635, L3272 등).
    //   - 자동 sync 가드(L3271)와 동일한 키를 사용해 부활 여부 일관 검증.
    for (const row of labor) {
      const r = row as any;
      if (!r.isLinkedFromRecovery) continue;
      const deletionKey = makeLinkedLaborDeletionKey(
        r.sourceAreaRowId || '',
        r.category || '',
        r.workName || '',
      ...
      if (deletedKeys.has(deletionKey)) {
        violations.push(`위험⑧: 삭제 키 부활 (key=${deletionKey}, loc=${r.location ?? ''})`);
      }
    }
    ...
    //     없는 노무비 행을 가리키는 경우 (= demote가 누락된 진짜 orphan).
    const laborIds = new Set(labor.map(r => r.id));
    for (const row of material) {
      const r = row as any;
      const stillAuto = r.isAutoGenerated || r.isLinkedFromRecovery;
      if (!stillAuto) continue;
      if (r.sourceLaborRowId && !laborIds.has(r.sourceLaborRowId)) {
        violations.push(`위험⑩: 자재비 orphan auto행 (id=...${(row.id ?? '').slice(-8)}, srcId=...${(r.sourceLaborRowId ?? '').slice(-8)})`);
      }
    }
