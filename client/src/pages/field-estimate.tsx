...
export default function FieldEstimate() {
  ...
  const syncMaterialFromRecoveryArea = (forceUnlock: boolean = false) => {
    ...
    // [회귀 보강] 공종|공사명(bare) 기준 fallback 맵.
    //   레거시 데이터는 자재항목 포함 autoKey(예: 수상공사|도배|실크벽지)로 저장된 경우가 있는데,
    //   현재 sync는 다중매칭을 항상 ...|__NONE__로 만들어 lookup이 빗나가고, reconcile에서 stale로
    //   삭제→빈 행 재생성되며 사용자가 고른 자재명/단가가 풀린다. bare key 맵으로 레거시 행을 찾아
    //   새 __NONE__ 행으로 이관(자재명/단가 보존)하도록 한다.
    const existingAutoRowsByBareKey = new Map<string, MaterialRow>();
    existingAutoRows.forEach(row => {
      ...
      // bare key fallback 등록: 잠금 행 포함 모두 등록(잠금 행 중복은 claimedAutoRowIds로
      //   survivor 단계에서 억제). 동일 bare key 충돌 시 자재항목이 채워진(사용자 선택) 행 우선.
      {
        const bareKey = `${norm(row.공종)}|${normalizedName}`;
        const prev = existingAutoRowsByBareKey.get(bareKey);
        const rowHasItem = norm(row.자재항목) !== '';
        if (!prev || (norm(prev.자재항목) === '' && rowHasItem)) {
          existingAutoRowsByBareKey.set(bareKey, rowForMap);
        }
      }
    });
    ...
    // bare key fallback로 새 __NONE__ 행에 자재명/단가가 이관된(claimed) 기존 행 id 추적.
    //   reconcile에서 잠금 stale 행 보존(survivor)을 막아 동일 공사 중복 행 생성 방지.
    const claimedAutoRowIds = new Set<string>();
    ...
    workMap.forEach((data) => {
      ...
      if (matchingMaterials.length > 0) {
        ...
        materialsToCreate.forEach(materialOrNull => {
          ...
          // bare key fallback: 레거시 item-suffixed autoKey(예: ...|실크벽지) 행을 찾아 보존/이관.
          const existingRow = existingAutoRowsMap.get(autoKey) || existingAutoRowsMap.get(fallbackKey) || existingAutoRowsByBareKey.get(fallbackKey);
          // 찾은 기존 행은 새 __NONE__ 행으로 이관되므로 claimed로 기록(잠금 stale survivor 중복 방지).
          if (existingRow?.id) claimedAutoRowIds.add(existingRow.id);
    ...
    const deletedCount = targetAutoRows.filter(row => {
      ...
      if (isStale && row.lockedAtSave && !forceUnlock) {
        // 이미 새 __NONE__ 행으로 자재명/단가가 이관(claimed)된 행은 survivor로 재추가하지 않음(중복 방지).
        if (row.id && claimedAutoRowIds.has(row.id)) {
          return true;
        }
                  ...
                  >
                    <thead>
                      ...
                      >
                        ...
                        <th style={{ padding: "12px 8px", fontFamily: "Pretendard", fontSize: "14px", fontWeight: 600, color: "rgba(12, 12, 12, 0.6)", whiteSpace: "nowrap", width: isMobileApp ? "75px" : undefined }}>위치</th>
                        <th style={{ padding: "12px 8px", fontFamily: "Pretendard", fontSize: "14px", fontWeight: 600, color: "rgba(12, 12, 12, 0.6)", whiteSpace: "nowrap", width: isMobileApp ? "75px" : undefined }}>공종</th>
