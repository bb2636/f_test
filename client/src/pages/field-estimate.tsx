...
export default function FieldEstimate() {
  ...
  const syncMaterialFromRecoveryArea = () => {
    ...
    workMap.forEach((data) => {
      ...
      if (isFixedIlwidaega) {
        const norm = (v: any) => (v ?? "").toString().trim();
        const autoKey = `${norm(data.공종)}|${norm(data.공사명)}|__FIXED__`;
        nextAutoKeys.add(autoKey);
        ...
        const laborUnitPrice = ilwidaegaItem?.노임단가 || 0;
        const fixedTotal = ilwidaegaItem?.일위대가 || 0;
        const fixedQty = laborUnitPrice > 0 && fixedTotal > 0
          ? Math.round((fixedTotal / laborUnitPrice) * 10) / 10
          : 1;
        ...
        if (existingRow && existingRow.isOverridden) {
          ...
          console.log(`[자재비 집계] ${data.공종} ${data.공사명}: isOverridden, 사용자 값 보존`);
        } else if (existingRow) {
          resultRowsMap.set(autoKey, {
            ...existingRow,
            autoKey,
