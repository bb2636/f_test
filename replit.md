...
- **Field Survey Workflow**: A sequential process for managing field investigations.
  ...
  - **Documents Upload**: Document and photo upload system with persistent Object Storage. Upload flow uses presigned URL pattern: (1) `POST /api/documents/request-upload` returns presigned URL + storageKey, (2) client PUTs file directly to Object Storage, (3) `POST /api/documents/complete-upload` saves metadata only to DB. DB에 파일 바이너리(base64) 저장 금지. PDF 파일은 비동기 백그라운드 압축 (status: processing → ready). `multipart-upload`/`direct-upload`는 2MB 이하 소형 파일 전용 레거시. 스토리지 상태 확인: `GET /api/health/storage`. Download: `/api/documents/:id/image` (Object Storage 스트리밍 또는 레거시 base64 서빙).
