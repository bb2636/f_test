...
- **Field Survey Workflow**: A sequential process for managing field investigations for a selected case.
  ...
  - **Estimate Management**: Create and submit restoration cost estimates with dynamic calculation tables linked to master data, version tracking, and PDF/Excel export.
    ...
    - **Labor Rate Tiers (노임단가 적용비율)**: Configurable 13-tier rate structure based on C/D ratio stored in database. Accessible via "적용비율 설정" button in labor cost section. Tiers determine rate multipliers (e.g., 85%+ = 100%, 0%+ = 40%, etc.) with edit/reset functionality. 적용단가(pricePerSqm) = E (노임단가, DB 값). 수량(인) = I / E (합계 ÷ 적용단가). 합계 = I = F + H.
...
- **견적서 적용단가/수량(인) 계산 공식 수정**: 현장출동보고서와 동일하게 변경
  - 적용단가 = E (노임단가, DB 값) — 기존: E/D
  - 수량(인) = I / E (합계 ÷ 노임단가) — 기존: I / (E/D)
  - 테이블 헤더: "기준가(m²)" → "적용단가", "수량" → "수량(인)"
