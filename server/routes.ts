...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/cases/:caseId/field-survey", async (req, res) => {
    ...
    try {
      ...
      // [케이스별 독립 정책 2026-05-12] accidentCause, vocContent는 동기화에서 제외.
      //   사고원인/고객소리는 피해세대별로 다를 수 있으므로 각 케이스가 독립적으로 작성/저장하도록 허용.
  ...
  app.get("/api/field-surveys/:caseId/report", async (req, res) => {
    ...
    try {
      ...
      // 담당자(관리자) 이름 자동 조회 - GET /api/cases/:id 와 동일한 보강
      // (현장출동보고서 응답에 managerName이 빠져 클라이언트에서 "미정"으로 표시되던 문제 수정)
      if (caseData.managerId) {
        const managerUser = allUsers.find((u) => u.id === caseData.managerId);
        if (managerUser) {
          (caseData as any).managerName = managerUser.name;
    ...
    try {
      ...
      if (selectedDocumentIds && selectedDocumentIds.length > 0) {
        ...
        if (selectedDocs.length > 0) {
          ...
          const TBL_V_PAD = 4;
          const TBL_MAX_ADDR_LINES = 3;

          // 주소를 col2 폭에 맞춰 글자 단위 wrap
          // (한글은 공백 없이 길게 이어지므로 글자 단위로 끊어 줄바꿈 — 카테고리 칸 침범 방지)
          const wrapAddrLines = (
            text: string,
            maxW: number,
            maxLines: number,
            ...
            let cur = "";
            let idx = 0;
            // 모든 줄에서 폭 강제 (카테고리 칸 침범 방지). 마지막 줄에서만 잔여 시 ... 부착.
            ...
            // 잔여 글자가 남았다면 마지막 줄에 ... 부착 (폭 강제)
            if (idx < arr.length && lines.length > 0) {
          ...
          const computeTableHeaderHeight = (
            ...
            const lines = wrapAddrLines(addr, col2Width - 10, TBL_MAX_ADDR_LINES);
          ...
          const drawTableHeader = (
            ...
            // 주소 wrap → 필요 높이 산출 (height보다 크면 위로 확장; top 유지를 위해 y는 아래로 이동)
            const addrLines = wrapAddrLines(
              normalizedAddress,
              col2Width - 10,
              TBL_MAX_ADDR_LINES,
            );
            ...
            // 하이픈 간격 보정 함수 (사고번호/주소 공용, 임의 y 지원)
            const drawTextTight = (
              text: string,
              startX: number,
              drawY: number,
            ...
            // 사고번호 (수직 중앙)
            drawTextTight(normalizedAccidentNo, x + 5, centerY);
          ...
          const flushPendingImages = async () => {
            ...
            while (i < pendingImages.length) {
              ...
              if (isTwoPerPage) {
                ...
                // (주소가 길어 wrap되면 슬롯 헤더 높이를 키워 이미지 침범 방지)
                ...
                // 주소 wrap 시 헤더가 위로 확장 → 반환된 실제 높이로 imgHeight 보정
            ...
            if (isPdfFile(mimeType, fileName)) {
              ...
              try {
                ...
                for (let pageIdx = 0; pageIdx < pageCount; pageIdx++) {
                  ...
                  // 주소 wrap 시 헤더가 확장될 수 있으므로 사전에 실제 높이 계산
                  const actualHeaderH = computeTableHeaderHeight(
                    headerInfo,
                    width - MARGIN * 2,
                    HEADER_HEIGHT,
                  );
                  ...
                  const totalHeaderSpace =
                    MARGIN + actualHeaderH + PDF_HEADER_GAP;
    ...
    try {
      ...
      if (selectedDocumentIds && selectedDocumentIds.length > 0) {
        ...
        if (selectedDocs.length > 0) {
          ...
          // 테이블 형태 헤더 그리기 헬퍼 함수 (현장출동보고서 스타일) — 이메일 경로
          const TBL_TEXT_SIZE = 8;
          const TBL_LINE_HEIGHT = 11;
          const TBL_V_PAD = 4;
          const TBL_MAX_ADDR_LINES = 3;
